extern crate diesel;
extern crate rellm;
use rellm::logic::renew_subscriptions_of_type;
...
/// row at all) across all three `PurchaseType`s in one pass -- one binary rather than three, unlike
/// most other background jobs in this file, since the three types share one table
/// (`market_subscriptions.product_type` distinguishes them) and the exact same renewal logic
/// (`logic::market_renewal::renew_subscriptions_of_type`), just filtered differently.
pub fn main() {
    init_crypto();
    init_bin_logging();
