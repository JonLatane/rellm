...
export enum MediaConversion {
  ...
  /**
   * VIDEO_PREVIEW_THUMBNAIL_SMALL - For video media only: a `image/jpeg` poster frame captured (via `ffmpeg`) at
   * `MediaMetadata.video_preview_time_ms`, resized to fit within 320x320 px, preserving aspect
   * ratio (never upscaled) -- same dimension tier as `MEDIA_CONVERSION_SMALL`, just a still frame
   * of the video rather than a resized copy of it. Regenerated by the same `convert_media_sizes`
   * background job; deleted (and marked for regeneration) whenever `UpdateMedia` changes
   * `video_preview_time_ms`, since the existing frame no longer matches the requested time.
   */
  VIDEO_PREVIEW_THUMBNAIL_SMALL = 5,
  /**
   * VIDEO_PREVIEW_THUMBNAIL_MEDIUM - Same as `VIDEO_PREVIEW_THUMBNAIL_SMALL`, but resized to fit within 800x800 px -- the
   * `MEDIA_CONVERSION_MEDIUM` tier.
   */
  VIDEO_PREVIEW_THUMBNAIL_MEDIUM = 6,
  /**
   * VIDEO_PREVIEW_THUMBNAIL_LARGE - Same as `VIDEO_PREVIEW_THUMBNAIL_SMALL`, but resized to fit within 1600x1600 px -- the
   * `MEDIA_CONVERSION_LARGE` tier.
   */
  VIDEO_PREVIEW_THUMBNAIL_LARGE = 7,
  UNRECOGNIZED = -1,
}
...
export function mediaConversionFromJSON(object: any): MediaConversion {
  switch (object) {
    ...
    case 5:
    case "VIDEO_PREVIEW_THUMBNAIL_SMALL":
      return MediaConversion.VIDEO_PREVIEW_THUMBNAIL_SMALL;
    case 6:
    case "VIDEO_PREVIEW_THUMBNAIL_MEDIUM":
      return MediaConversion.VIDEO_PREVIEW_THUMBNAIL_MEDIUM;
    case 7:
    case "VIDEO_PREVIEW_THUMBNAIL_LARGE":
      return MediaConversion.VIDEO_PREVIEW_THUMBNAIL_LARGE;
    case -1:
    case "UNRECOGNIZED":
    default:
...
export function mediaConversionToJSON(object: MediaConversion): string {
  switch (object) {
    ...
    case MediaConversion.VIDEO_PREVIEW_THUMBNAIL_SMALL:
      return "VIDEO_PREVIEW_THUMBNAIL_SMALL";
    case MediaConversion.VIDEO_PREVIEW_THUMBNAIL_MEDIUM:
      return "VIDEO_PREVIEW_THUMBNAIL_MEDIUM";
    case MediaConversion.VIDEO_PREVIEW_THUMBNAIL_LARGE:
      return "VIDEO_PREVIEW_THUMBNAIL_LARGE";
...
export const MediaMetadata: MessageFns<MediaMetadata> = {
  encode(message: MediaMetadata, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.videoPreviewTimeMs !== undefined) {
      writer.uint32(8).uint64(message.videoPreviewTimeMs);
    }
    return writer;
  },
    ...
    while (reader.pos < end) {
      ...
      switch (tag >>> 3) {
        case 1: {
          ...
          message.videoPreviewTimeMs = longToNumber(reader.uint64());
          continue;
        }
      }
      if ((tag & 7) === 4 || tag === 0) {
