...
module Proto.Rellm.Rellm exposing
    ( accessToken, cancelMarketSubscription, configureServer, createAIProvider, createAccount, createEvent, createFollow, createGroup
    , createGroupPost, createMarketProduct, createMembership, createNewOccasions, createPost, createSyncDestination, createSyncSource
    , defederateProfile, deleteAIProvider, deleteEvent, deleteEventAttendance, deleteFollow, deleteGroup, deleteGroupPost, deleteMedia
    , deleteMediaSizes, deleteMembership, deleteOccasionSyncDestination, deletePost, deletePostSyncDestination, deleteRemovedOccasions
    , deleteSyncDestination, deleteSyncSource, deleteUser, federateProfile, freeClusterResources, generateMedia, getAIProviders
    , getCurrentUser, getEventAttendances, getEvents, getGroupPosts, getGroups, getMarketProducts, getMarketSubscriptions, getMedia
    , getMembers, getMessages, getPosts, getPushSubscriptionStatus, getServerConfiguration, getServiceVersion, getSyncDestinations
    , getSyncSources, getUsers, grantAIProvider, lockClusterResources, login, makeMarketPurchase, markMessagesRead, registerPushSubscription
    , resetData, resetPassword, revokeAIProvider, sendMessage, starPost, startContactMethodVerification, syncOccasion, syncPost
    , unregisterPushSubscription, unstarPost, updateAIProvider, updateEvent, updateEventDetails, updateFollow, updateGroup, updateGroupPost
    , updateMarketProduct, updateMarketSubscription, updateMedia, updateMembership, updateOccasions, updatePost, updateSyncDestination
    , updateSyncSource, updateUser, upsertEventAttendance, verifyContactMethod
    )
...
{-| This file was automatically generated by
  ...
  - `protoc` 5.29.3
  - the following specification files: `rellm.proto`
...
Rellm servers interact across several ports:
  ...
  - HTTP (80, 8000, 27705), HTTPS (443) - The main Rellm HTTP API. This is used for some endpoints, including media upload/download, and for negotiating the gRPC host.
      - [Public HTTP Endpoints](#external-http-servers-80-8000-443) and the Elm, React and Flutter [Web UI](#web-ui-paths)s are served up on multiple ports:
          ...
          - Port 8000 _always_ serves up an unsecured Rellm UI, in case something goes horribly wrong with 80 and 443. It can probably not be exposed in your load balancer/to the web.
...
one-directional (reading in, not posting out) - publishing a Rellm [`Post`](#rellm-Post) _to_
...
just to _read_ public posts.

Connecting an actual Mastodon _account_ is a heavier flow, since Mastodon has no single central
...
The use of composition over inheritance also means that Rellm APIs can be _predictably_ non-atomic based on their compositional structure.
...
Rellm incorporates server configuration, including fairly deep customization of the end-user UI/UX, as perhaps its _most_ primitive type.
...
and is more like a document than a row in a database. (That said, every [`ServerConfiguration`](#rellm-ServerConfiguration) change _is_ a row in a database, meaning
...
Unlike a real `FederatedServer`, though, an entry here is _not_ required just to browse an
instance's public timeline read-only - that's already a public, unauthenticated Mastodon REST
endpoint any client can call directly. It's only needed to let a user _connect their own_
...
app (`app_id`/`app_secret`, the latter _never_ serialized to the client) on each instance
...
outgoing pushes and is _never_ serialized to clients - like the federation secrets above, admins set/rotate it
...
Threads API is a product added to this server's _existing_ Facebook App (see [`FacebookAuthConfig`](#rellm-FacebookAuthConfig)) rather than a
...
content, it's metered _access_ an owner can share out to other users of this server. As with
...
used for image _generation only_ (no editing - DigitalOcean's Serverless Inference API has no
...
`(ai_provider_id, grantee)` pair - granting again _resets_, rather than adds to, `tokens_remaining`) and
...
content itself is _not_ served over gRPC - it's uploaded/downloaded via plain HTTP (`POST`/`GET /media`) - while
...
An [`Event`](#rellm-Event) is a wrapper for _at least two_ [`Post`](#rellm-Post)s. It always has its own top-level [`Post`](#rellm-Post)
(`PostContext.EVENT`, holding the event's overall title/description) _and_ it must have at least one
...
First, before _any_ authentication is done, you should [resolve your backend host](#http-based-client-host-negotiation-for-external-cdns-get-backend_host),
...
have (or are willing to create) on a _different_ Rellm server, without either backend ever seeing a plaintext
...
4.  The user authenticates via the [`Login`](#grpc-api-Login) RPC. This always issues a _fresh_ `refresh_token`/
...
The UI will only show federated profiles if _both use profiles_ have federated with one another.
...
current user is actually an Admin _on that server_.
...
links to its own in-app product page as usual, but a tile for any _other_ server links straight to that
...
an oversized body returns `413 Payload Too Large`.
  ...
  - **Response**: `200 OK` with a body of `{"action": "accept"}` on success - Stalwart's MTA Hook protocol parses
    the response _body_, not just the status code, so this has to be the exact shape it expects
...
to `jonline.io.itsj.online` on port 27707/443 instead. To users, the server should still _generally_ appear to
...
(an `OPTIONS /media` variant also exists, solely to satisfy CORS preflight requests). _Authenticated_ (via
...
Tamagui and Elm share one page structure (below) and are always _both_ reachable, explicitly, at `/tamagui/*`
...
`Pages.Auth.To.Key_`. Reached only via the cross-origin redirect from step 2 above (built by the _requesting_
origin's Accounts panel), never linked to directly.
  ...
  - **Behavior**: shows a sign-in form for _this_ server (or a "currently signed in as ..." badge, if already
...
@docs accessToken, cancelMarketSubscription, configureServer, createAIProvider, createAccount, createEvent, createFollow, createGroup

@docs createGroupPost, createMarketProduct, createMembership, createNewOccasions, createPost, createSyncDestination, createSyncSource

@docs defederateProfile, deleteAIProvider, deleteEvent, deleteEventAttendance, deleteFollow, deleteGroup, deleteGroupPost, deleteMedia

@docs deleteMediaSizes, deleteMembership, deleteOccasionSyncDestination, deletePost, deletePostSyncDestination, deleteRemovedOccasions

@docs deleteSyncDestination, deleteSyncSource, deleteUser, federateProfile, freeClusterResources, generateMedia, getAIProviders

@docs getCurrentUser, getEventAttendances, getEvents, getGroupPosts, getGroups, getMarketProducts, getMarketSubscriptions, getMedia

@docs getMembers, getMessages, getPosts, getPushSubscriptionStatus, getServerConfiguration, getServiceVersion, getSyncDestinations

@docs getSyncSources, getUsers, grantAIProvider, lockClusterResources, login, makeMarketPurchase, markMessagesRead, registerPushSubscription

@docs resetData, resetPassword, revokeAIProvider, sendMessage, starPost, startContactMethodVerification, syncOccasion, syncPost

@docs unregisterPushSubscription, unstarPost, updateAIProvider, updateEvent, updateEventDetails, updateFollow, updateGroup, updateGroupPost

@docs updateMarketProduct, updateMarketSubscription, updateMedia, updateMembership, updateOccasions, updatePost, updateSyncDestination

@docs updateSyncSource, updateUser, upsertEventAttendance, verifyContactMethod
...
Delete ALL Media, Posts, Groups and Users except the user who performed the RPC. _Authenticated._
...
{-| A template for a gRPC call to the method 'FreeClusterResources' sending a `FreeClusterResourcesRequest` to get back a `Empty`.

Releases resources previously acquired via
[`LockClusterResources`](#grpc-api-LockClusterResources). _Publicly accessible **or**
Authenticated_ - unlike `LockClusterResources`, this accepts _either_ the
`cluster-shared-secret` header (same as `LockClusterResources`) _or_ normal per-user auth, in
...
-}
freeClusterResources : Grpc.Internal.Rpc Proto.Rellm.FreeClusterResourcesRequest Proto.Google.Protobuf.Empty
freeClusterResources =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        , rpcName = "FreeClusterResources"
        , encoder = Proto.Rellm.encodeFreeClusterResourcesRequest
        , decoder = Proto.Google.Protobuf.decodeEmpty
        }


{-| A template for a gRPC call to the method 'LockClusterResources' sending a `LockClusterResourcesRequest` to get back a `LockClusterResourcesResponse`.

Attempts to acquire one or more `ClusterResource` locks on behalf of `namespace_id`. _Not
part of the authenticated-user auth system_ - this is server-to-server, cluster-internal
coordination, authorized _entirely_ by the `cluster-shared-secret` gRPC metadata header
...
-}
lockClusterResources : Grpc.Internal.Rpc Proto.Rellm.LockClusterResourcesRequest Proto.Rellm.LockClusterResourcesResponse
lockClusterResources =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        , rpcName = "LockClusterResources"
        , encoder = Proto.Rellm.encodeLockClusterResourcesRequest
        , decoder = Proto.Rellm.decodeLockClusterResourcesResponse
        }
...
[`GetServerConfiguration`](#grpc-api-GetServerConfiguration)). _Authenticated._ Requires
...
-}
configureServer : Grpc.Internal.Rpc Proto.Rellm.ServerConfiguration Proto.Rellm.ServerConfiguration
configureServer =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        , rpcName = "ConfigureServer"
        , encoder = Proto.Rellm.encodeServerConfiguration
        , decoder = Proto.Rellm.decodeServerConfiguration
        }
...
-}
defederateProfile : Grpc.Internal.Rpc Proto.Rellm.FederatedAccount Proto.Google.Protobuf.Empty
defederateProfile =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        , rpcName = "DefederateProfile"
        , encoder = Proto.Rellm.encodeFederatedAccount
        , decoder = Proto.Google.Protobuf.decodeEmpty
        }
...
Federate the current user's profile with another user profile. _Authenticated_.

-}
federateProfile : Grpc.Internal.Rpc Proto.Rellm.FederatedAccount Proto.Rellm.FederatedAccount
federateProfile =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        ...
        , encoder = Proto.Rellm.encodeFederatedAccount
        , decoder = Proto.Rellm.decodeFederatedAccount
        }


{-| A template for a gRPC call to the method 'DeleteEventAttendance' sending a `EventAttendance` to get back a `Empty`.

Delete an EventAttendance. _Publicly accessible **or** Authenticated, with anonymous RSVP support._

-}
deleteEventAttendance : Grpc.Internal.Rpc Proto.Rellm.EventAttendance Proto.Google.Protobuf.Empty
deleteEventAttendance =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        , rpcName = "DeleteEventAttendance"
        , encoder = Proto.Rellm.encodeEventAttendance
        , decoder = Proto.Google.Protobuf.decodeEmpty
        }
...
Upsert an EventAttendance. _Publicly accessible **or** Authenticated, with anonymous RSVP support._
...
-}
upsertEventAttendance : Grpc.Internal.Rpc Proto.Rellm.EventAttendance Proto.Rellm.EventAttendance
upsertEventAttendance =
    Grpc.Internal.Rpc
        ...
        , encoder = Proto.Rellm.encodeEventAttendance
        , decoder = Proto.Rellm.decodeEventAttendance
        }
...
Gets EventAttendances for an Occasion. _Publicly accessible **or** Authenticated._

-}
getEventAttendances : Grpc.Internal.Rpc Proto.Rellm.GetEventAttendancesRequest Proto.Rellm.EventAttendances
getEventAttendances =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        , rpcName = "GetEventAttendances"
        , encoder = Proto.Rellm.encodeGetEventAttendancesRequest
        , decoder = Proto.Rellm.decodeEventAttendances
        }


{-| A template for a gRPC call to the method 'GenerateMedia' sending a `GenerateMediaRequest` to get back a `Media`.

Generates (or edits, given reference `media_ids`) an image via one of the current user's AIModels,
storing it as a new Media and, if `target` is set, attaching it to that Post/Event. _Authenticated_ - caller
...
AIProviderGrant.tokens\_remaining on every call - the provider's own reported token usage once generation
...
-}
generateMedia : Grpc.Internal.Rpc Proto.Rellm.GenerateMediaRequest Proto.Rellm.Media
generateMedia =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        , rpcName = "GenerateMedia"
        , encoder = Proto.Rellm.encodeGenerateMediaRequest
        , decoder = Proto.Rellm.decodeMedia
        }


{-| A template for a gRPC call to the method 'UpdateMarketSubscription' sending a `MarketSubscription` to get back a `MarketSubscription`.
...
to set it independently of a note. _Authenticated_, requires Admin (for now -- see that field's
own doc on why this may loosen to "buyer, or Admin" later).

-}
updateMarketSubscription : Grpc.Internal.Rpc Proto.Rellm.MarketSubscription Proto.Rellm.MarketSubscription
updateMarketSubscription =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        , rpcName = "UpdateMarketSubscription"
        , encoder = Proto.Rellm.encodeMarketSubscription
        , decoder = Proto.Rellm.decodeMarketSubscription
        }


{-| A template for a gRPC call to the method 'CancelMarketSubscription' sending a `MarketSubscription` to get back a `MarketSubscription`.
...
`service_terminated_at`, once both have passed. _Authenticated_ -- the subscription's own buyer,
or an Admin.

-}
cancelMarketSubscription : Grpc.Internal.Rpc Proto.Rellm.MarketSubscription Proto.Rellm.MarketSubscription
cancelMarketSubscription =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        , rpcName = "CancelMarketSubscription"
        , encoder = Proto.Rellm.encodeMarketSubscription
        , decoder = Proto.Rellm.decodeMarketSubscription
        }


{-| A template for a gRPC call to the method 'MakeMarketPurchase' sending a `MakeMarketPurchaseRequest` to get back a `MakeMarketPurchaseResponse`.

Starts (or resumes) buying a MarketProduct for the current user, returning a Stripe Checkout URL to
redirect to. _Authenticated_. See `MakeMarketPurchaseRequest`'s own doc -- no MarketPurchase/
MarketSubscription is created by this call itself, only once Stripe confirms payment via webhook.

-}
makeMarketPurchase : Grpc.Internal.Rpc Proto.Rellm.MakeMarketPurchaseRequest Proto.Rellm.MakeMarketPurchaseResponse
makeMarketPurchase =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        , rpcName = "MakeMarketPurchase"
        , encoder = Proto.Rellm.encodeMakeMarketPurchaseRequest
        , decoder = Proto.Rellm.decodeMakeMarketPurchaseResponse
        }


{-| A template for a gRPC call to the method 'GetMarketSubscriptions' sending a `GetMarketSubscriptionsRequest` to get back a `GetMarketSubscriptionsResponse`.
...
see `RellmHostingSubscriptionDetails.fulfillment_status`'s own doc) for an Admin. _Authenticated_.

-}
getMarketSubscriptions : Grpc.Internal.Rpc Proto.Rellm.GetMarketSubscriptionsRequest Proto.Rellm.GetMarketSubscriptionsResponse
getMarketSubscriptions =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        , rpcName = "GetMarketSubscriptions"
        , encoder = Proto.Rellm.encodeGetMarketSubscriptionsRequest
        , decoder = Proto.Rellm.decodeGetMarketSubscriptionsResponse
        }


{-| A template for a gRPC call to the method 'UpdateMarketProduct' sending a `MarketProduct` to get back a `MarketProduct`.

Updates a MarketProduct's amount/currency/details/delisted\_at. _Authenticated_, requires Admin.
`type`/`period` are immutable after creation and are ignored if changed.

-}
updateMarketProduct : Grpc.Internal.Rpc Proto.Rellm.MarketProduct Proto.Rellm.MarketProduct
updateMarketProduct =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        , rpcName = "UpdateMarketProduct"
        , encoder = Proto.Rellm.encodeMarketProduct
        , decoder = Proto.Rellm.decodeMarketProduct
        }


{-| A template for a gRPC call to the method 'CreateMarketProduct' sending a `MarketProduct` to get back a `MarketProduct`.

Creates a MarketProduct. _Authenticated_, requires Admin.

-}
createMarketProduct : Grpc.Internal.Rpc Proto.Rellm.MarketProduct Proto.Rellm.MarketProduct
createMarketProduct =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        , rpcName = "CreateMarketProduct"
        , encoder = Proto.Rellm.encodeMarketProduct
        , decoder = Proto.Rellm.decodeMarketProduct
        }


{-| A template for a gRPC call to the method 'GetMarketProducts' sending a `GetMarketProductsRequest` to get back a `GetMarketProductsResponse`.

Gets MarketProducts available for purchase on this server (`market.proto`). _Unauthenticated_ --
admins additionally see delisted MarketProducts.

-}
getMarketProducts : Grpc.Internal.Rpc Proto.Rellm.GetMarketProductsRequest Proto.Rellm.GetMarketProductsResponse
getMarketProducts =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        , rpcName = "GetMarketProducts"
        , encoder = Proto.Rellm.encodeGetMarketProductsRequest
        , decoder = Proto.Rellm.decodeGetMarketProductsResponse
        }


{-| A template for a gRPC call to the method 'RevokeAIProvider' sending a `RevokeAIProviderRequest` to get back a `Empty`.

Revokes another user's access to one of the current user's AIProviders. _Authenticated_, owner-only (no Admin override).

-}
revokeAIProvider : Grpc.Internal.Rpc Proto.Rellm.RevokeAIProviderRequest Proto.Google.Protobuf.Empty
revokeAIProvider =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        , rpcName = "RevokeAIProvider"
        , encoder = Proto.Rellm.encodeRevokeAIProviderRequest
        , decoder = Proto.Google.Protobuf.decodeEmpty
        }


{-| A template for a gRPC call to the method 'GrantAIProvider' sending a `GrantAIProviderRequest` to get back a `AIProviderGrant`.

Grants (or resets) another user's metered access to one of the current user's AIProviders. _Authenticated_, owner-only (no Admin override).

-}
grantAIProvider : Grpc.Internal.Rpc Proto.Rellm.GrantAIProviderRequest Proto.Rellm.AIProviderGrant
grantAIProvider =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        , rpcName = "GrantAIProvider"
        , encoder = Proto.Rellm.encodeGrantAIProviderRequest
        , decoder = Proto.Rellm.decodeAIProviderGrant
        }


{-| A template for a gRPC call to the method 'DeleteAIProvider' sending a `DeleteAIProviderRequest` to get back a `Empty`.

Deletes an AIProvider (and its AIProviderGrants). _Authenticated_ (owner, or Admin).

-}
deleteAIProvider : Grpc.Internal.Rpc Proto.Rellm.DeleteAIProviderRequest Proto.Google.Protobuf.Empty
deleteAIProvider =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        , rpcName = "DeleteAIProvider"
        , encoder = Proto.Rellm.encodeDeleteAIProviderRequest
        , decoder = Proto.Google.Protobuf.decodeEmpty
        }


{-| A template for a gRPC call to the method 'UpdateAIProvider' sending a `AIProvider` to get back a `AIProvider`.

Updates an AIProvider's name, provider, or credentials. _Authenticated_ (owner, or Admin for any user's).

-}
updateAIProvider : Grpc.Internal.Rpc Proto.Rellm.AIProvider Proto.Rellm.AIProvider
updateAIProvider =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        , rpcName = "UpdateAIProvider"
        , encoder = Proto.Rellm.encodeAIProvider
        , decoder = Proto.Rellm.decodeAIProvider
        }


{-| A template for a gRPC call to the method 'CreateAIProvider' sending a `AIProvider` to get back a `AIProvider`.

Creates an AIProvider for the current user. _Authenticated_, requires `CREATE_AI_PROVIDERS` (or Admin).

-}
createAIProvider : Grpc.Internal.Rpc Proto.Rellm.AIProvider Proto.Rellm.AIProvider
createAIProvider =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        , rpcName = "CreateAIProvider"
        , encoder = Proto.Rellm.encodeAIProvider
        , decoder = Proto.Rellm.decodeAIProvider
        }


{-| A template for a gRPC call to the method 'GetAIProviders' sending a `User` to get back a `GetAIProvidersResponse`.

Gets a user's AIProviders. _Authenticated_ (self, or Admin for any user).

-}
getAIProviders : Grpc.Internal.Rpc Proto.Rellm.User Proto.Rellm.GetAIProvidersResponse
getAIProviders =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        , rpcName = "GetAIProviders"
        , encoder = Proto.Rellm.encodeUser
        , decoder = Proto.Rellm.decodeGetAIProvidersResponse
        }


{-| A template for a gRPC call to the method 'DeleteOccasionSyncDestination' sending a `DeleteOccasionSyncDestinationRequest` to get back a `Empty`.

Removes an Occasion's sync (cross-post) to a SyncDestination, the reverse of [`SyncOccasion`](#grpc-api-SyncOccasion). _Authenticated_ (destination owner, or Admin), requires `SYNC_EVENTS_TO_FACEBOOK` (or Admin).

-}
deleteOccasionSyncDestination : Grpc.Internal.Rpc Proto.Rellm.DeleteOccasionSyncDestinationRequest Proto.Google.Protobuf.Empty
deleteOccasionSyncDestination =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        , rpcName = "DeleteOccasionSyncDestination"
        , encoder = Proto.Rellm.encodeDeleteOccasionSyncDestinationRequest
        , decoder = Proto.Google.Protobuf.decodeEmpty
        }


{-| A template for a gRPC call to the method 'SyncOccasion' sending a `SyncOccasionRequest` to get back a `Occasion`.

Syncs (cross-posts) an Occasion to a SyncDestination. _Authenticated_ (destination owner, or Admin), requires `SYNC_EVENTS_TO_FACEBOOK` (or Admin).

-}
syncOccasion : Grpc.Internal.Rpc Proto.Rellm.SyncOccasionRequest Proto.Rellm.Occasion
syncOccasion =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        , rpcName = "SyncOccasion"
        , encoder = Proto.Rellm.encodeSyncOccasionRequest
        , decoder = Proto.Rellm.decodeOccasion
        }


{-| A template for a gRPC call to the method 'DeleteSyncDestination' sending a `DeleteSyncDestinationRequest` to get back a `Empty`.

Deletes a SyncDestination. _Authenticated_ (owner, or Admin).

-}
deleteSyncDestination : Grpc.Internal.Rpc Proto.Rellm.DeleteSyncDestinationRequest Proto.Google.Protobuf.Empty
deleteSyncDestination =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        , rpcName = "DeleteSyncDestination"
        , encoder = Proto.Rellm.encodeDeleteSyncDestinationRequest
        , decoder = Proto.Google.Protobuf.decodeEmpty
        }


{-| A template for a gRPC call to the method 'UpdateSyncDestination' sending a `SyncDestination` to get back a `SyncDestination`.

Updates a SyncDestination. _Authenticated_ (owner, or Admin for any user's), requires `SYNC_EVENTS_TO_FACEBOOK` or `SYNC_POSTS_TO_FACEBOOK` (or Admin).

-}
updateSyncDestination : Grpc.Internal.Rpc Proto.Rellm.SyncDestination Proto.Rellm.SyncDestination
updateSyncDestination =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        , rpcName = "UpdateSyncDestination"
        , encoder = Proto.Rellm.encodeSyncDestination
        , decoder = Proto.Rellm.decodeSyncDestination
        }


{-| A template for a gRPC call to the method 'CreateSyncDestination' sending a `SyncDestination` to get back a `SyncDestination`.

Creates a SyncDestination for the current user. _Authenticated_, requires `SYNC_EVENTS_TO_FACEBOOK` or `SYNC_POSTS_TO_FACEBOOK` (or Admin).

-}
createSyncDestination : Grpc.Internal.Rpc Proto.Rellm.SyncDestination Proto.Rellm.SyncDestination
createSyncDestination =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        , rpcName = "CreateSyncDestination"
        , encoder = Proto.Rellm.encodeSyncDestination
        , decoder = Proto.Rellm.decodeSyncDestination
        }


{-| A template for a gRPC call to the method 'GetSyncDestinations' sending a `User` to get back a `GetSyncDestinationsResponse`.

Gets a user's SyncDestinations. _Authenticated_ (self, or Admin for any user).

-}
getSyncDestinations : Grpc.Internal.Rpc Proto.Rellm.User Proto.Rellm.GetSyncDestinationsResponse
getSyncDestinations =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        , rpcName = "GetSyncDestinations"
        , encoder = Proto.Rellm.encodeUser
        , decoder = Proto.Rellm.decodeGetSyncDestinationsResponse
        }


{-| A template for a gRPC call to the method 'DeleteSyncSource' sending a `DeleteSyncSourceRequest` to get back a `Empty`.

Deletes a SyncSource. _Authenticated_ (owner, or Admin).

-}
deleteSyncSource : Grpc.Internal.Rpc Proto.Rellm.DeleteSyncSourceRequest Proto.Google.Protobuf.Empty
deleteSyncSource =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        , rpcName = "DeleteSyncSource"
        , encoder = Proto.Rellm.encodeDeleteSyncSourceRequest
        , decoder = Proto.Google.Protobuf.decodeEmpty
        }
...
Updates a SyncSource. _Authenticated_ (owner, or Admin for any user's), requires
...
-}
updateSyncSource : Grpc.Internal.Rpc Proto.Rellm.SyncSource Proto.Rellm.SyncSource
updateSyncSource =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        , rpcName = "UpdateSyncSource"
        , encoder = Proto.Rellm.encodeSyncSource
        , decoder = Proto.Rellm.decodeSyncSource
        }


{-| A template for a gRPC call to the method 'CreateSyncSource' sending a `SyncSource` to get back a `SyncSource`.

Creates a SyncSource for the current user. _Authenticated_, requires `SYNC_EVENTS_FROM_ICS`/
`SYNC_POSTS_FROM_RSS`/`SYNC_POSTS_FROM_ATOM` (whichever matches `configuration`, or Admin).

-}
createSyncSource : Grpc.Internal.Rpc Proto.Rellm.SyncSource Proto.Rellm.SyncSource
createSyncSource =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        , rpcName = "CreateSyncSource"
        , encoder = Proto.Rellm.encodeSyncSource
        , decoder = Proto.Rellm.decodeSyncSource
        }


{-| A template for a gRPC call to the method 'GetSyncSources' sending a `User` to get back a `GetSyncSourcesResponse`.

Gets a user's SyncSources. _Authenticated_ (self, or Admin for any user).

-}
getSyncSources : Grpc.Internal.Rpc Proto.Rellm.User Proto.Rellm.GetSyncSourcesResponse
getSyncSources =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        , rpcName = "GetSyncSources"
        , encoder = Proto.Rellm.encodeUser
        , decoder = Proto.Rellm.decodeGetSyncSourcesResponse
        }


{-| A template for a gRPC call to the method 'DeleteRemovedOccasions' sending a `Event` to get back a `Event`.

Deletes Occasions in an existing Event that aren't present in the input Event. _Authenticated._

-}
deleteRemovedOccasions : Grpc.Internal.Rpc Proto.Rellm.Event Proto.Rellm.Event
deleteRemovedOccasions =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        , rpcName = "DeleteRemovedOccasions"
        , encoder = Proto.Rellm.encodeEvent
        , decoder = Proto.Rellm.decodeEvent
        }
...
Any other Occasions in the request are ignored. _Authenticated._

-}
updateOccasions : Grpc.Internal.Rpc Proto.Rellm.Event Proto.Rellm.Event
updateOccasions =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        , rpcName = "UpdateOccasions"
        , encoder = Proto.Rellm.encodeEvent
        , decoder = Proto.Rellm.decodeEvent
        }
...
Creates Occasions in an existing Event for every Occasion in the request that isn't already on the event. _Authenticated._
Any other Occasions in the request are ignored.

-}
createNewOccasions : Grpc.Internal.Rpc Proto.Rellm.Event Proto.Rellm.Event
createNewOccasions =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        , rpcName = "CreateNewOccasions"
        , encoder = Proto.Rellm.encodeEvent
        , decoder = Proto.Rellm.decodeEvent
        }


{-| A template for a gRPC call to the method 'UpdateEventDetails' sending a `Event` to get back a `Event`.

Updates only the [`Event`](#rellm-Event)'s top-level details and those of its [`Post`](#rellm-Post) (not any [`Occasion`](#rellm-Occasion)s or their [`Post`](#rellm-Post)s). _Authenticated._

-}
updateEventDetails : Grpc.Internal.Rpc Proto.Rellm.Event Proto.Rellm.Event
updateEventDetails =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        , rpcName = "UpdateEventDetails"
        , encoder = Proto.Rellm.encodeEvent
        , decoder = Proto.Rellm.decodeEvent
        }
...
(Soft) deletes a Event. Returns the deleted version of the Event. _Authenticated._

-}
deleteEvent : Grpc.Internal.Rpc Proto.Rellm.Event Proto.Rellm.Event
deleteEvent =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
...
Updates an Event. Automatically creates/updates/deletes child Occasions of the Event. _Authenticated._
...
-}
updateEvent : Grpc.Internal.Rpc Proto.Rellm.Event Proto.Rellm.Event
updateEvent =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
...
Creates an Event. _Authenticated._

-}
createEvent : Grpc.Internal.Rpc Proto.Rellm.Event Proto.Rellm.Event
createEvent =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
...
Gets Events. _Publicly accessible **or** Authenticated._
Unauthenticated calls only return Events of `GLOBAL_PUBLIC` visibility.

-}
getEvents : Grpc.Internal.Rpc Proto.Rellm.GetEventsRequest Proto.Rellm.GetEventsResponse
getEvents =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        , rpcName = "GetEvents"
        , encoder = Proto.Rellm.encodeGetEventsRequest
        , decoder = Proto.Rellm.decodeGetEventsResponse
        }


{-| A template for a gRPC call to the method 'DeleteGroupPost' sending a `GroupPost` to get back a `Empty`.

Delete a GroupPost. _Authenticated._

-}
deleteGroupPost : Grpc.Internal.Rpc Proto.Rellm.GroupPost Proto.Google.Protobuf.Empty
deleteGroupPost =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        , rpcName = "DeleteGroupPost"
        , encoder = Proto.Rellm.encodeGroupPost
        , decoder = Proto.Google.Protobuf.decodeEmpty
        }
...
Group Moderators: Approve/Reject a GroupPost. _Authenticated._

-}
updateGroupPost : Grpc.Internal.Rpc Proto.Rellm.GroupPost Proto.Rellm.GroupPost
updateGroupPost =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        ...
        , encoder = Proto.Rellm.encodeGroupPost
        , decoder = Proto.Rellm.decodeGroupPost
        }


{-| A template for a gRPC call to the method 'CreateGroupPost' sending a `GroupPost` to get back a `GroupPost`.

Cross-post a Post to a Group. _Authenticated._

-}
createGroupPost : Grpc.Internal.Rpc Proto.Rellm.GroupPost Proto.Rellm.GroupPost
createGroupPost =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
...
Get GroupPosts for a Post (and optional group). _Publicly accessible **or** Authenticated._

-}
getGroupPosts : Grpc.Internal.Rpc Proto.Rellm.GetGroupPostsRequest Proto.Rellm.GetGroupPostsResponse
getGroupPosts =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        , rpcName = "GetGroupPosts"
        , encoder = Proto.Rellm.encodeGetGroupPostsRequest
        , decoder = Proto.Rellm.decodeGetGroupPostsResponse
        }


{-| A template for a gRPC call to the method 'DeletePostSyncDestination' sending a `DeletePostSyncDestinationRequest` to get back a `Empty`.

Removes a Post's sync (cross-post) to a SyncDestination, the reverse of [`SyncPost`](#grpc-api-SyncPost). _Authenticated_ (destination owner, or Admin), requires `SYNC_POSTS_TO_FACEBOOK` (or Admin).

-}
deletePostSyncDestination : Grpc.Internal.Rpc Proto.Rellm.DeletePostSyncDestinationRequest Proto.Google.Protobuf.Empty
deletePostSyncDestination =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        , rpcName = "DeletePostSyncDestination"
        , encoder = Proto.Rellm.encodeDeletePostSyncDestinationRequest
        , decoder = Proto.Google.Protobuf.decodeEmpty
        }


{-| A template for a gRPC call to the method 'SyncPost' sending a `SyncPostRequest` to get back a `Post`.

Syncs (cross-posts) a Post to a SyncDestination. _Authenticated_ (destination owner, or Admin), requires `SYNC_POSTS_TO_FACEBOOK` (or Admin).

-}
syncPost : Grpc.Internal.Rpc Proto.Rellm.SyncPostRequest Proto.Rellm.Post
syncPost =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        , rpcName = "SyncPost"
        , encoder = Proto.Rellm.encodeSyncPostRequest
        , decoder = Proto.Rellm.decodePost
        }


{-| A template for a gRPC call to the method 'UnstarPost' sending a `Post` to get back a `Post`.

Unstar a Post. _Unauthenticated._

-}
unstarPost : Grpc.Internal.Rpc Proto.Rellm.Post Proto.Rellm.Post
unstarPost =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        , rpcName = "UnstarPost"
        , encoder = Proto.Rellm.encodePost
        , decoder = Proto.Rellm.decodePost
        }
...
Star a Post. _Unauthenticated._

-}
starPost : Grpc.Internal.Rpc Proto.Rellm.Post Proto.Rellm.Post
starPost =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
...
(TODO) (Soft) deletes a Post. Returns the deleted version of the Post. _Authenticated._

-}
deletePost : Grpc.Internal.Rpc Proto.Rellm.Post Proto.Rellm.Post
deletePost =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
...
Updates a Post. _Authenticated._

-}
updatePost : Grpc.Internal.Rpc Proto.Rellm.Post Proto.Rellm.Post
updatePost =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
...
Creates a Post. _Authenticated._

-}
createPost : Grpc.Internal.Rpc Proto.Rellm.Post Proto.Rellm.Post
createPost =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
...
Gets Posts. _Publicly accessible **or** Authenticated._
Unauthenticated calls only return Posts of `GLOBAL_PUBLIC` visibility.

-}
getPosts : Grpc.Internal.Rpc Proto.Rellm.GetPostsRequest Proto.Rellm.GetPostsResponse
getPosts =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        , rpcName = "GetPosts"
        , encoder = Proto.Rellm.encodeGetPostsRequest
        , decoder = Proto.Rellm.decodeGetPostsResponse
        }


{-| A template for a gRPC call to the method 'DeleteMembership' sending a `Membership` to get back a `Empty`.

Leave a group (or cancel membership request). _Authenticated._

-}
deleteMembership : Grpc.Internal.Rpc Proto.Rellm.Membership Proto.Google.Protobuf.Empty
deleteMembership =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        , rpcName = "DeleteMembership"
        , encoder = Proto.Rellm.encodeMembership
        , decoder = Proto.Google.Protobuf.decodeEmpty
        }
...
Update aspects of a user's membership. _Authenticated._
...
-}
updateMembership : Grpc.Internal.Rpc Proto.Rellm.Membership Proto.Rellm.Membership
updateMembership =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        ...
        , encoder = Proto.Rellm.encodeMembership
        , decoder = Proto.Rellm.decodeMembership
        }


{-| A template for a gRPC call to the method 'CreateMembership' sending a `Membership` to get back a `Membership`.

Requests to join a group (or joins it), or sends an invite to the user. _Authenticated._
Memberships and moderations are set to their defaults.

-}
createMembership : Grpc.Internal.Rpc Proto.Rellm.Membership Proto.Rellm.Membership
createMembership =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
...
Get Members (User+Membership) of a Group. _Publicly accessible **or** Authenticated._

-}
getMembers : Grpc.Internal.Rpc Proto.Rellm.GetMembersRequest Proto.Rellm.GetMembersResponse
getMembers =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        , rpcName = "GetMembers"
        , encoder = Proto.Rellm.encodeGetMembersRequest
        , decoder = Proto.Rellm.decodeGetMembersResponse
        }


{-| A template for a gRPC call to the method 'DeleteGroup' sending a `Group` to get back a `Empty`.

Delete a Group. _Authenticated._
Requires `ADMIN` permissions within the group, or `ADMIN` permissions for the user.

-}
deleteGroup : Grpc.Internal.Rpc Proto.Rellm.Group Proto.Google.Protobuf.Empty
deleteGroup =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        , rpcName = "DeleteGroup"
        , encoder = Proto.Rellm.encodeGroup
        , decoder = Proto.Google.Protobuf.decodeEmpty
        }
...
Update a Groups's information, default membership permissions or moderation. _Authenticated._
Requires `ADMIN` permissions within the group, or `ADMIN` permissions for the user.

-}
updateGroup : Grpc.Internal.Rpc Proto.Rellm.Group Proto.Rellm.Group
updateGroup =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        , rpcName = "UpdateGroup"
        , encoder = Proto.Rellm.encodeGroup
        , decoder = Proto.Rellm.decodeGroup
        }


{-| A template for a gRPC call to the method 'CreateGroup' sending a `Group` to get back a `Group`.

Creates a group with the current user as its admin. _Authenticated._
Requires the `CREATE_GROUPS` permission.

-}
createGroup : Grpc.Internal.Rpc Proto.Rellm.Group Proto.Rellm.Group
createGroup =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
...
Gets Groups. _Publicly accessible **or** Authenticated._
Unauthenticated calls only return Groups of `GLOBAL_PUBLIC` visibility.

-}
getGroups : Grpc.Internal.Rpc Proto.Rellm.GetGroupsRequest Proto.Rellm.GetGroupsResponse
getGroups =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        , rpcName = "GetGroups"
        , encoder = Proto.Rellm.encodeGetGroupsRequest
        , decoder = Proto.Rellm.decodeGetGroupsResponse
        }


{-| A template for a gRPC call to the method 'DeleteFollow' sending a `Follow` to get back a `Empty`.

Unfollow (or unrequest) a user. _Authenticated._

-}
deleteFollow : Grpc.Internal.Rpc Proto.Rellm.Follow Proto.Google.Protobuf.Empty
deleteFollow =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        , rpcName = "DeleteFollow"
        , encoder = Proto.Rellm.encodeFollow
        , decoder = Proto.Google.Protobuf.decodeEmpty
        }
...
Used to approve follow requests. _Authenticated._

-}
updateFollow : Grpc.Internal.Rpc Proto.Rellm.Follow Proto.Rellm.Follow
updateFollow =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        ...
        , encoder = Proto.Rellm.encodeFollow
        , decoder = Proto.Rellm.decodeFollow
        }


{-| A template for a gRPC call to the method 'CreateFollow' sending a `Follow` to get back a `Follow`.

Follow (or request to follow) a user. _Authenticated._

-}
createFollow : Grpc.Internal.Rpc Proto.Rellm.Follow Proto.Rellm.Follow
createFollow =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
...
{-| A template for a gRPC call to the method 'GetPushSubscriptionStatus' sending a `GetPushSubscriptionStatusRequest` to get back a `GetPushSubscriptionStatusResponse`.

Checks whether the calling user specifically (not just "some account on this browser") has a
[`PushSubscription`](#rellm-PushSubscription) registered for `endpoint`. _Authenticated._ Exists because a browser only
ever exposes its own subscription's `endpoint`/keys, never _who_ on the server side is
...
-}
getPushSubscriptionStatus : Grpc.Internal.Rpc Proto.Rellm.GetPushSubscriptionStatusRequest Proto.Rellm.GetPushSubscriptionStatusResponse
getPushSubscriptionStatus =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        , rpcName = "GetPushSubscriptionStatus"
        , encoder = Proto.Rellm.encodeGetPushSubscriptionStatusRequest
        , decoder = Proto.Rellm.decodeGetPushSubscriptionStatusResponse
        }


{-| A template for a gRPC call to the method 'UnregisterPushSubscription' sending a `UnregisterPushSubscriptionRequest` to get back a `Empty`.

Unregisters a browser's Web Push subscription, e.g. on logout or when `PushManager.subscribe()`
reports the subscription as no longer valid. _Authenticated._ Not an error if `endpoint` isn't
currently registered to the calling user.

-}
unregisterPushSubscription : Grpc.Internal.Rpc Proto.Rellm.UnregisterPushSubscriptionRequest Proto.Google.Protobuf.Empty
unregisterPushSubscription =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        , rpcName = "UnregisterPushSubscription"
        , encoder = Proto.Rellm.encodeUnregisterPushSubscriptionRequest
        , decoder = Proto.Google.Protobuf.decodeEmpty
        }


{-| A template for a gRPC call to the method 'RegisterPushSubscription' sending a `RegisterPushSubscriptionRequest` to get back a `PushSubscription`.
...
browser tab is closed. _Authenticated._ Re-registering an already-registered `endpoint` (e.g.
...
[`WebPushConfig`](#rellm-WebPushConfig) configured - there's nothing to push notifications _with_.

-}
registerPushSubscription : Grpc.Internal.Rpc Proto.Rellm.RegisterPushSubscriptionRequest Proto.Rellm.PushSubscription
registerPushSubscription =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        , rpcName = "RegisterPushSubscription"
        , encoder = Proto.Rellm.encodeRegisterPushSubscriptionRequest
        , decoder = Proto.Rellm.decodePushSubscription
        }
...
thread once it's been opened. _Authenticated._ Only needs the recipient/sender access
[`GetMessages`](#grpc-api-GetMessages) already requires for each Message - no separate permission. Atomic: if the
caller lacks access to _any_ of `message_ids`, none of them are marked (matching
`MarkMessagesReadRequest.message_ids`' own doc), so a client never has to reconcile a
partially-applied batch.

-}
markMessagesRead : Grpc.Internal.Rpc Proto.Rellm.MarkMessagesReadRequest Proto.Rellm.MarkMessagesReadResponse
markMessagesRead =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        , rpcName = "MarkMessagesRead"
        , encoder = Proto.Rellm.encodeMarkMessagesReadRequest
        , decoder = Proto.Rellm.decodeMarkMessagesReadResponse
        }


{-| A template for a gRPC call to the method 'GetMessages' sending a `GetMessagesRequest` to get back a `GetMessagesResponse`.

Gets Messages. _Authenticated._
...
-}
getMessages : Grpc.Internal.Rpc Proto.Rellm.GetMessagesRequest Proto.Rellm.GetMessagesResponse
getMessages =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        , rpcName = "GetMessages"
        , encoder = Proto.Rellm.encodeGetMessagesRequest
        , decoder = Proto.Rellm.decodeGetMessagesResponse
        }


{-| A template for a gRPC call to the method 'SendMessage' sending a `SendMessageRequest` to get back a `Message`.

Sends a Message to one or more recipients (creating/reusing their MessagingGroup). _Publicly
accessible **or** Authenticated._ Like [`CreatePost`](#grpc-api-CreatePost)/[`CreateEvent`](#grpc-api-CreateEvent), authentication (if any) is via
a standard `access_token`; unauthenticated calls are simply sent with no `sender`.

-}
sendMessage : Grpc.Internal.Rpc Proto.Rellm.SendMessageRequest Proto.Rellm.Message
sendMessage =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        , rpcName = "SendMessage"
        , encoder = Proto.Rellm.encodeSendMessageRequest
        , decoder = Proto.Rellm.decodeMessage
        }


{-| A template for a gRPC call to the method 'DeleteUser' sending a `User` to get back a `Empty`.

Deletes a user by ID. _Authenticated._
Deleting other users requires `ADMIN` permissions.

-}
deleteUser : Grpc.Internal.Rpc Proto.Rellm.User Proto.Google.Protobuf.Empty
deleteUser =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
...
Verifies a code sent by [`StartContactMethodVerification`](#grpc-api-StartContactMethodVerification). _Authenticated, self-only._
...
-}
verifyContactMethod : Grpc.Internal.Rpc Proto.Rellm.VerifyContactMethodRequest Proto.Rellm.ContactMethod
verifyContactMethod =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        , rpcName = "VerifyContactMethod"
        , encoder = Proto.Rellm.encodeVerifyContactMethodRequest
        , decoder = Proto.Rellm.decodeContactMethod
        }
...
[`ContactMethod`](#rellm-ContactMethod). _Authenticated, self-only._ Requires the server to
...
-}
startContactMethodVerification : Grpc.Internal.Rpc Proto.Rellm.ContactMethod Proto.Rellm.ContactMethod
startContactMethodVerification =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        , rpcName = "StartContactMethodVerification"
        , encoder = Proto.Rellm.encodeContactMethod
        , decoder = Proto.Rellm.decodeContactMethod
        }
...
Update a user by ID. _Authenticated._
Updating other users requires `ADMIN` permissions.

-}
updateUser : Grpc.Internal.Rpc Proto.Rellm.User Proto.Rellm.User
updateUser =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        , rpcName = "UpdateUser"
        , encoder = Proto.Rellm.encodeUser
        , decoder = Proto.Rellm.decodeUser
        }


{-| A template for a gRPC call to the method 'GetUsers' sending a `GetUsersRequest` to get back a `GetUsersResponse`.

Gets Users. _Publicly accessible **or** Authenticated._
Unauthenticated calls only return Users of `GLOBAL_PUBLIC` visibility.

-}
getUsers : Grpc.Internal.Rpc Proto.Rellm.GetUsersRequest Proto.Rellm.GetUsersResponse
getUsers =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        , rpcName = "GetUsers"
        , encoder = Proto.Rellm.encodeGetUsersRequest
        , decoder = Proto.Rellm.decodeGetUsersResponse
        }


{-| A template for a gRPC call to the method 'DeleteMediaSizes' sending a `Media` to get back a `Media`.
...
in its place. _Authenticated._ Deleting other users' media requires `ADMIN` permissions.
Errors if this would leave the Media item with no `sizes` at all -- use `DeleteMedia` to
remove the whole item instead.

-}
deleteMediaSizes : Grpc.Internal.Rpc Proto.Rellm.Media Proto.Rellm.Media
deleteMediaSizes =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        , rpcName = "DeleteMediaSizes"
        , encoder = Proto.Rellm.encodeMedia
        , decoder = Proto.Rellm.decodeMedia
        }


{-| A template for a gRPC call to the method 'UpdateMedia' sending a `Media` to get back a `Media`.

Updates a Media item's `name`/`description`/`metadata.video_preview_time_ms` by ID.
_Authenticated._ Every other field (visibility, moderation, `sizes`, etc.) is ignored -- use
...
-}
updateMedia : Grpc.Internal.Rpc Proto.Rellm.Media Proto.Rellm.Media
updateMedia =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
...
Deletes a media item by ID. _Authenticated._ Note that media may still be accessible for 12 hours after deletes are requested, as separate jobs clean it up from S3/MinIO.
Deleting other users' media requires `ADMIN` permissions.

-}
deleteMedia : Grpc.Internal.Rpc Proto.Rellm.Media Proto.Google.Protobuf.Empty
deleteMedia =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
...
Gets Media (Images, Videos, etc) uploaded/owned by the current user. _Authenticated._ To upload/download actual Media blob/binary data, use the [HTTP Media APIs](#media).

-}
getMedia : Grpc.Internal.Rpc Proto.Rellm.GetMediaRequest Proto.Rellm.GetMediaResponse
getMedia =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        , rpcName = "GetMedia"
        , encoder = Proto.Rellm.encodeGetMediaRequest
        , decoder = Proto.Rellm.decodeGetMediaResponse
        }


{-| A template for a gRPC call to the method 'ResetPassword' sending a `ResetPasswordRequest` to get back a `Empty`.

Resets the current user's - or, for admins, a given user's - password. _Authenticated._

-}
resetPassword : Grpc.Internal.Rpc Proto.Rellm.ResetPasswordRequest Proto.Google.Protobuf.Empty
resetPassword =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        , rpcName = "ResetPassword"
        , encoder = Proto.Rellm.encodeResetPasswordRequest
        , decoder = Proto.Google.Protobuf.decodeEmpty
        }
...
Gets the current user. _Authenticated._

-}
getCurrentUser : Grpc.Internal.Rpc Proto.Google.Protobuf.Empty Proto.Rellm.User
getCurrentUser =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
...
Gets a new `access_token` (and possibly a new `refresh_token`, which should replace the old one in client storage), given a `refresh_token`. _Publicly accessible._

-}
accessToken : Grpc.Internal.Rpc Proto.Rellm.AccessTokenRequest Proto.Rellm.AccessTokenResponse
accessToken =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        , rpcName = "AccessToken"
        , encoder = Proto.Rellm.encodeAccessTokenRequest
        , decoder = Proto.Rellm.decodeAccessTokenResponse
        }


{-| A template for a gRPC call to the method 'Login' sending a `LoginRequest` to get back a `RefreshTokenResponse`.

Logs in a user and provides a `refresh_token` (along with an `access_token`). _Publicly accessible._

-}
login : Grpc.Internal.Rpc Proto.Rellm.LoginRequest Proto.Rellm.RefreshTokenResponse
login =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        , rpcName = "Login"
        , encoder = Proto.Rellm.encodeLoginRequest
        , decoder = Proto.Rellm.decodeRefreshTokenResponse
        }


{-| A template for a gRPC call to the method 'CreateAccount' sending a `CreateAccountRequest` to get back a `RefreshTokenResponse`.

Creates a user account and provides a `refresh_token` (along with an `access_token`). _Publicly accessible._

-}
createAccount : Grpc.Internal.Rpc Proto.Rellm.CreateAccountRequest Proto.Rellm.RefreshTokenResponse
createAccount =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        , rpcName = "CreateAccount"
        , encoder = Proto.Rellm.encodeCreateAccountRequest
        , decoder = Proto.Rellm.decodeRefreshTokenResponse
        }
...
Gets the Rellm server's [`ServerConfiguration`](#rellm-ServerConfiguration). _Publicly
accessible_ -- some fields (e.g. `twilio_config`, `preferred_verification_apis`) are stripped
for a non-admin caller, see that message's own field docs.

-}
getServerConfiguration : Grpc.Internal.Rpc Proto.Google.Protobuf.Empty Proto.Rellm.ServerConfiguration
...
Get the version (from Cargo) of the Rellm service. _Publicly accessible._

-}
getServiceVersion : Grpc.Internal.Rpc Proto.Google.Protobuf.Empty Proto.Rellm.GetServiceVersionResponse
getServiceVersion =
    Grpc.Internal.Rpc
        { service = "Rellm"
        , package = "rellm"
        ...
        , encoder = Proto.Google.Protobuf.encodeEmpty
        , decoder = Proto.Rellm.decodeGetServiceVersionResponse
        }
